...
const createCollege = async (req, res) => {
    try {
        ...
        if (!name) {
            return res
                .status(400).send({ status: false, message: "Name must contain" })
        }
        ...
        if (!fullName) {
            return res.
                status(400).send({ status: false, message: "please provide fullname" })
        }
        if (!validator.isValidCharacterLimit2to100(fullName)) {
            return res.
                status(400).send({ status: false, message: "please Enter a valid fullname" })
        }
        ...
        if (!logoLink) {
            return res.
                status(400).send({ status: false, message: "LogoLink must contain" })
        }
        if (!validator.isValidUrl(logoLink)) {
            return res.
                status(400).send({ status: false, message: "please Enter a valid logolink." })
        }
